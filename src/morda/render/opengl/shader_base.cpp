...
program_wrapper::program_wrapper(const char *vertex_shader_code,
  ...
  if (check_for_link_errors(this->p)) {
    LOG([&](auto &o) {
      o << "Error while linking shader program" << vertex_shader_code
        << std::endl
...
shader_base::shader_base(const char *vertex_shader_code,
                         const char *fragment_shader_code)
...
void shader_base::render(const r4::matrix4<float> &m,
                         const morda::vertex_array &va) const {
  ASSERT(this->is_bound())

  ASSERT(dynamic_cast<const index_buffer *>(&va.indices.get()))
  const index_buffer &ivbo =
      static_cast<const index_buffer &>(va.indices.get());
  ...
  glDrawElements(mode_to_gl_mode(va.rendering_mode), ivbo.elements_count,
                 ivbo.element_type, nullptr);
  assert_opengl_no_error();
