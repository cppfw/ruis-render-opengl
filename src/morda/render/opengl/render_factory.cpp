...
render_factory::create_texture_2d(const rasterimage::image_variant &imvar) {
  return std::visit(
      [this, &imvar](const auto &im) {
        auto data = im.pixels();
        return this->create_texture_2d_internal(
            imvar.get_format(), im.dims(),
            utki::make_span(reinterpret_cast<const uint8_t *>(data.data()),
                            data.size_bytes()));
      },
      imvar.variant);
}
...
utki::shared_ref<morda::texture_2d> render_factory::create_texture_2d_internal(
    rasterimage::format type, rasterimage::dimensioned::dimensions_type dims,
  ...
  ASSERT(data.size() == 0 ||
         data.size() / rasterimage::to_num_channels(type) / dims.x() ==
             dims.y())
  ...
  glTexImage2D(GL_TEXTURE_2D,
               ...
               GL_UNSIGNED_BYTE, data.size() == 0 ? nullptr : data.data());
  assert_opengl_no_error();
...
render_factory::create_vertex_buffer(utki::span<const float> vertices) {
  return utki::make_shared<vertex_buffer>(vertices);
}
...
render_factory::create_index_buffer(utki::span<const uint16_t> indices) {
  return utki::make_shared<index_buffer>(indices);
}

std::unique_ptr<morda::render_factory::shaders>
render_factory::create_shaders() {
